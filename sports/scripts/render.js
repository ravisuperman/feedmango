...
async function renderNews() {
  ...
  // ── SPORTS SHORTS ────────────────────────────────────────────────────────
  // Pick top 6-8 articles with images for the "Shorts" rail
  const railWrapper = document.getElementById('shortsRailWrapper');
  const rail = document.getElementById('shortsRail');
  if (railWrapper && rail) {
    // We only show shorts on 'main' or 'ipl' feed, or if there's enough content
        ...
        railHtml += `
          <div class="short-bubble" onclick="openShortsViewer(${idx})">
            <div class="short-avatar-ring">
              <div class="short-avatar" style="font-size:12px; font-weight:800; color:#000;">${init}</div>
            </div>
