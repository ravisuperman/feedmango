...
async function renderNews() {
  ...
  // ── SPORTS SHORTS ────────────────────────────────────────────────────────
  // Only show the "Shorts" rail on the Main (Top Stories) feed
  const railWrapper = document.getElementById('shortsRailWrapper');
  const rail = document.getElementById('shortsRail');
  if (railWrapper && rail) {
    if (currentSport !== 'main') {
      railWrapper.style.display = 'none';
    } else {
      const shortCandidates = interleaved.filter(x => x.image);
      if (shortCandidates.length >= 4) {
          ...
          railHtml += `
            <div class="short-bubble" onclick="openShortsViewer(${idx})">
              <div class="short-avatar-ring">
                <div class="short-avatar" style="font-size:11px; font-weight:800; color:var(--text); letter-spacing:-0.5px;">${abbr}</div>
              </div>
            </div>
          `;
        });
        rail.innerHTML = railHtml;
        railWrapper.style.display = 'block';
        // Init left-arrow visibility for mobile scroll detection
        if (typeof initShortsNavVisibility === 'function') initShortsNavVisibility();
      } else {
        railWrapper.style.display = 'none';
      }
    }
  ...
  // ── SCOREBOARD & POINTS TABLE VISIBILITY ──────────────────────────────────
  // Only show the scoreboard and points table on Main, IPL, and Cricket
  const sbWrapper = document.getElementById('scoreboardWrapper');
  const ptsBar = document.getElementById('ptsLinkBar');
  const showScoreboard = ['main', 'ipl', 'cricket'].includes(currentSport);
  
  if (sbWrapper && sbWrapper.innerHTML.trim() !== '') {
    sbWrapper.style.display = showScoreboard ? 'flex' : 'none';
  }
  if (ptsBar) {
    ptsBar.style.display = showScoreboard ? 'flex' : 'none';
  }
  // ─────────────────────────────────────────────────────────────────────────
...
function scrollScoreboard(dir) {
  ...
  if (isMobile) {
    // On mobile: snap exactly one card width at a time
    const card = scroll.querySelector('.score-card');
    const cardWidth = card ? card.offsetWidth + 16 : scroll.clientWidth; // card + gap
    scroll.scrollBy({ left: dir * cardWidth, behavior: 'smooth' });
  } else {
    // Desktop: scroll by ~1.5 cards
...
function renderScoreboardTicker(allMatches) {
  ...
  // ── MOBILE BOTTOM SHEET: populate & show (once per session) ─────────────────
  if (window.innerWidth <= 768) {
    const sheetScroll = document.getElementById('schedSheetScroll');
    if (sheetScroll) {
      sheetScroll.innerHTML = html; // Same cards, full-width inside the sheet
    }
    if (typeof showScheduleSheet === 'function') {
      showScheduleSheet();
    }
  }
      ...
      if (matchTimeMs > 0 || sClass === 'live') {
        ...
        const renderBmc = () => {
          if (sClass === 'live' || Date.now() >= matchTimeMs) {
            ...
            bmcContainer.innerHTML = `<span>Now: </span><span class="bmc-teams">${t1} vs ${t2}</span><span class="bmc-timer live">MATCH LIVE</span>`;
            if (window._bmcInterval) clearInterval(window._bmcInterval);
          } else {
