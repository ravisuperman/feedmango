...
(function() {
  ...
  window.addEventListener('scroll', function() {
    ...
    if (isMobile()) {
      ...
      header.classList.remove('scrolled'); // Ensure shrunk state resets
    } else {
      header.classList.remove('hidden');
      floatMenu.classList.remove('visible');
      ...
      // Desktop Shrinking Header Effect
      if (currentScroll > 10) {
...
function renderPointsTable(standings, container) {
  ...
  standings.forEach((t, i) => {
    ...
    // Fix API abbreviation
    if (t.teamShort === 'RCBW') t.teamShort = 'RCB';
...
/**
 * ═══════════════════════════════════════════════════════════
 * SPORTS SHORTS (Instagram Style)
 * ═══════════════════════════════════════════════════════════
 */
let currentShortIdx = 0;
let shortsInterval = null;
let shortsTimeout = null;
const SHORT_DURATION = 5000; // 5 seconds per short

function openShortsViewer(index) {
  if (!window._shortsData || !window._shortsData[index]) return;
  currentShortIdx = index;
  ...
  // Build progress bars
  const pContainer = document.getElementById('shortsProgressContainer');
  pContainer.innerHTML = '';
  window._shortsData.forEach((_, i) => {
    pContainer.innerHTML += `
      <div class="shorts-progress-bar">
        <div class="shorts-progress-fill" id="shortProgressFill_${i}"></div>
      </div>
    `;
  });
  
  // Mark the avatar ring as viewed
  const rings = document.querySelectorAll('.short-avatar-ring');
  if (rings[index]) rings[index].classList.add('viewed');
  
  renderCurrentShort();
}

function renderCurrentShort() {
  const short = window._shortsData[currentShortIdx];
  if (!short) return;
  
  // Reset all progress bars
  window._shortsData.forEach((_, i) => {
    const fill = document.getElementById(`shortProgressFill_${i}`);
    if (fill) {
      fill.style.transition = 'none';
      if (i < currentShortIdx) fill.style.width = '100%';
      else fill.style.width = '0%';
    }
  });
  
  // Populate UI
  document.getElementById('shortsBg').style.backgroundImage = `url(${short.image})`;
  document.getElementById('shortsTitle').innerText = short.title;
  document.getElementById('shortsDesc').innerText = cleanText(short.description || '');
  document.getElementById('shortsLink').href = short.link;
  
  const source = short.source || 'SPORTSrip';
  document.getElementById('shortsAuthorName').innerText = source;
  document.getElementById('shortsTime').innerText = getTimeAgo(short.pubDate);
  
  const init = source.substring(0, 2).toUpperCase();
  const authorImg = document.getElementById('shortsAuthorImg');
  if (short.authorPhoto) {
    authorImg.src = short.authorPhoto;
  ...
  const activeFill = document.getElementById(`shortProgressFill_${currentShortIdx}`);
  if (activeFill) {
    // Force reflow
    void activeFill.offsetWidth;
    activeFill.style.transition = `width ${SHORT_DURATION}ms linear`;
    activeFill.style.width = '100%';
  }
  
  // Auto advance
  clearTimeout(shortsTimeout);
  shortsTimeout = setTimeout(() => {
    nextShort();
  }, SHORT_DURATION);
}

function nextShort() {
  if (currentShortIdx < window._shortsData.length - 1) {
    currentShortIdx++;
...
function prevShort() {
  if (currentShortIdx > 0) {
    ...
    renderCurrentShort(); // Restart current
  }
}
...
// On mobile: show/hide the left arrow based on scroll position
// Called after the shorts rail is dynamically rendered
function initShortsNavVisibility() {
  const rail = document.getElementById('shortsRail');
  const leftBtn = document.querySelector('.shorts-nav-left');
  if (!rail || !leftBtn) return;

  const updateLeftArrow = () => {
    if (window.innerWidth <= 768) {
      leftBtn.classList.toggle('visible', rail.scrollLeft > 4);
    } else {
      leftBtn.classList.add('visible'); // always show on desktop
    }
  };

  // Remove old listener to avoid duplicates on re-render
  rail.removeEventListener('scroll', rail._leftArrowHandler);
  rail._leftArrowHandler = updateLeftArrow;
  rail.addEventListener('scroll', updateLeftArrow, { passive: true });
  window.addEventListener('resize', updateLeftArrow);
  updateLeftArrow(); // set initial state
}

// Helper: time ago for shorts header
function getTimeAgo(dateStr) {
  if (!dateStr) return '';
  const diff = Date.now() - new Date(dateStr).getTime();
  if (diff < 0) return 'Just now'; // Handle future timezone quirks
...
// Session-only flag — resets every page refresh, never written to localStorage
window._scheduleSheetSeen = false;

function showScheduleSheet() {
  // Only on mobile, only once per session
  if (window._scheduleSheetSeen) return;
  if (window.innerWidth > 768) return;

  const sheet    = document.getElementById('schedSheet');
  const backdrop = document.getElementById('schedBackdrop');
  if (!sheet || !backdrop) return;

  window._scheduleSheetSeen = true;

  // Make both visible first (still off-screen / transparent)
  sheet.style.display    = 'block';
  backdrop.style.display = 'block';

  // Force a reflow so the CSS transitions fire
...
function closeScheduleSheet() {
  ...
  // Wait for slide-down animation to finish, then hide
  setTimeout(() => {
    sheet.style.display    = 'none';
    backdrop.style.display = 'none';
    document.body.style.overflow = '';
  }, 370); // matches the 0.35s CSS transition
}
