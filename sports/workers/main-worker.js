...
// VAPID_PUBLIC_KEY:  stored here (safe to expose)
// VAPID_PRIVATE_KEY: stored as Cloudflare secret binding
//   Set via: wrangler secret put VAPID_PRIVATE_KEY
//   Value:   YBTqE_WwHXCdn6nJ4ZhXqTBakMdbJPQU20qBcT9A03g
// ============================================================
var VAPID_PUBLIC_KEY  = 'BJOaptbSZUsRzMezX2wxFmc3CbH2Z7xrWIlE6SWgpugo-I2hZLW0A_2UmwHHwc0jxco1ivTT1kXEk9e7coveL1Q';
var VAPID_SUBJECT     = 'mailto:ravi.kompel@gmail.com';
...
// Build a VAPID JWT signed with the private key
async function buildVapidJWT(audience, privateKeyB64, expSeconds) {
  const header  = base64UrlEncode(new TextEncoder().encode(JSON.stringify({ typ: 'JWT', alg: 'ES256' })));
  const now     = Math.floor(Date.now() / 1000);
  const payload = base64UrlEncode(new TextEncoder().encode(JSON.stringify({
    aud: audience,
    exp: now + (expSeconds || 43200),
    sub: VAPID_SUBJECT
  })));
  const sigInput = header + '.' + payload;

  // Import the raw EC private key bytes
  ...
  const cryptoKey = await crypto.subtle.importKey(
    ...
    // Fallback: try importing as pkcs8 (some environments need this)
...
// Send a Web Push message to one subscriber
async function sendWebPush(subscription, payload, privateKeyB64) {
  try {
    const endpoint = subscription.endpoint;
    const audience = new URL(endpoint).origin;
    const jwt      = await buildVapidJWT(audience, privateKeyB64);
    const vapidAuth = `vapid t=${jwt},k=${VAPID_PUBLIC_KEY}`;

    const body = JSON.stringify(payload);
    // Encrypt the payload (simple unencrypted for broad compat; most real cases use web-push lib)
...
async function sendPushToSubscribers(env, payload) {
  if (!env.PUSH_KV) return { sent: 0, failed: 0, note: 'PUSH_KV not bound' };
  const privateKey = env.VAPID_PRIVATE_KEY;
  if (!privateKey) return { sent: 0, failed: 0, note: 'VAPID_PRIVATE_KEY secret not set' };

  var raw = await env.PUSH_KV.get('push_subscriptions');
  var subscriptions = raw ? JSON.parse(raw) : [];
  if (!subscriptions.length) return { sent: 0, failed: 0, note: 'No subscribers yet' };

  var sent = 0, failed = 0, expired = [];
  await Promise.allSettled(subscriptions.map(async function(sub) {
    var result = await sendWebPush(sub, payload, privateKey);
    if (result.ok) {
      sent++;
  ...
  if (expired.length) {
    var cleaned = subscriptions.filter(function(s) { return !expired.includes(s.endpoint); });
    await env.PUSH_KV.put('push_subscriptions', JSON.stringify(cleaned));
  }
...
export default {
  async fetch(request,env){
    ...
    // ── /api/subscribe — Save push subscription token ────────────────────────
    if (url.pathname === '/api/subscribe' && request.method === 'POST') {
      if (!env.PUSH_KV) return new Response(JSON.stringify({ error: 'PUSH_KV not configured' }), { status: 503, headers: cors });
      var body = await request.json();
      var sub  = body.subscription;
      if (!sub || !sub.endpoint) return new Response(JSON.stringify({ error: 'Invalid subscription' }), { status: 400, headers: cors });

      var raw   = await env.PUSH_KV.get('push_subscriptions');
      var subs  = raw ? JSON.parse(raw) : [];
      // Deduplicate by endpoint
      var exists = subs.some(function(s) { return s.endpoint === sub.endpoint; });
      if (!exists) {
        subs.push(sub);
        await env.PUSH_KV.put('push_subscriptions', JSON.stringify(subs));
      }
      return new Response(JSON.stringify({ success: true, total: subs.length }), { headers: cors });
    }

    // ── /api/push — Admin broadcast push to all subscribers ──────────────────
    if (url.pathname === '/api/push' && request.method === 'POST') {
      if (!isAdmin(request)) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers: cors });
      var body    = await request.json();
      var payload = {
        title: body.title || 'SPORTSrip 🏏',
        body:  body.body  || 'New sports update!',
        url:   body.url   || '/',
        tag:   body.tag   || 'sportsrip-manual',
        icon:  '/favicon.ico'
      };
      var result = await sendPushToSubscribers(env, payload);
      return new Response(JSON.stringify({ success: true, ...result }), { headers: cors });
    }

    // ── /api/subscribers — Count subscribers (admin only) ────────────────────
    if (url.pathname === '/api/subscribers') {
      if (!isAdmin(request)) return new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401, headers: cors });
      if (!env.PUSH_KV) return new Response(JSON.stringify({ count: 0, note: 'PUSH_KV not bound' }), { headers: cors });
      var raw  = await env.PUSH_KV.get('push_subscriptions');
      var subs = raw ? JSON.parse(raw) : [];
      return new Response(JSON.stringify({ count: subs.length }), { headers: cors });
    }
  ...
  async scheduled(event, env, ctx) {
    ...
    if (event.cron === '30 23 * * *' || event.cron === '30 11 * * *') {
      // Offset cron: fetch raw → curate with OG images → SEO agent → push notifications
      ctx.waitUntil((async function () {
        // Snapshot top IPL headline before refresh (to detect new articles)
        var prevIPL = '';
        try {
          var prevRaw = await env.CURATED_KV.get('curated:ipl');
          var prevArr = prevRaw ? JSON.parse(prevRaw) : [];
          if (prevArr.length) prevIPL = prevArr[0].title || '';
        } catch(e) {}
        ...
        // ── Auto Push: fire notification if top IPL headline changed ──
        try {
          var newRaw = await env.CURATED_KV.get('curated:ipl');
          var newArr = newRaw ? JSON.parse(newRaw) : [];
          if (newArr.length && newArr[0].title && newArr[0].title !== prevIPL) {
            var top = newArr[0];
            await sendPushToSubscribers(env, {
              title: '🏏 SPORTSrip — IPL Update',
              body:  top.title.slice(0, 100),
              url:   '/?tab=ipl',
              tag:   'sportsrip-ipl-' + Date.now(),
              icon:  '/favicon.ico'
            });
          }
        } catch(e) {
          console.error('[Push] Auto-push failed:', e.message);
        }
      })());
    } else {
