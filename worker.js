...
export default {
  ...
  async fetch(request,env){
    ...
    if(url.pathname==='/api/mode')return new Response(JSON.stringify({mode:'auto',version:'v3.0-standardized',deployed:'01-Apr-2026',worker:CONFIG.workerName}),{headers:cors});
    ...
    if(url.pathname==='/api/preload-feeds'&&request.method==='POST'){
      ...
      PRELOADED_FEEDS.forEach(function(f){
        if(!existingUrls.has(f.url)){ existing.push({...f, addedAt:new Date().toISOString()}); added++; }
      });
