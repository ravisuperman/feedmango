...
function renderScoreboardTicker(allMatches) {
  ...
  // IST = UTC + 5h30m (330 minutes). cricAPI dateTimeGMT is always UTC.
  const toIST = (gmtStr) => {
    if (!gmtStr) return null;
    // Normalise: cricAPI sometimes sends "2026-04-19 14:30:00" without T/Z
    const normalised = gmtStr.includes('T') ? gmtStr : gmtStr.replace(' ', 'T') + 'Z';
    const utc = new Date(normalised);
    if (isNaN(utc)) return null;
    // Add 330 min offset manually so we always get IST regardless of browser timezone
  ...
  const getStatus = (m) => {
    ...
    // Upcoming — build IST time label
    const ist = toIST(m.dateTimeGMT);
    if (ist) {
      const now = new Date();
      // Compare dates using IST date string (midnight-aligned)
      const nowIST = new Date(now.getTime() + 330 * 60000);
      const isToday    = ist.date.toUTCString().slice(0,16) === nowIST.toUTCString().slice(0,16);
      const isTomorrow = ist.date.toUTCString().slice(0,16) === new Date(nowIST.getTime() + 86400000).toUTCString().slice(0,16);
      if (isToday)    return { sClass: 'upcoming', sText: `TODAY ${ist.timeStr}` };
      if (isTomorrow) return { sClass: 'upcoming', sText: `TOMORROW ${ist.timeStr}` };
      // Further future: show date + time IST
  ...
  // Strategy: Index-based calculation avoids all offsetLeft / offsetParent issues.
  // We measure ONE card's actual rendered width using getBoundingClientRect 
  // (width is reliable even inside scroll containers), then multiply by the
  // target card's index plus gap to get exact scrollLeft.
  setTimeout(() => {
    const allCards = Array.from(scroll.querySelectorAll('.score-card'));
    const targetIdx = allCards.findIndex(
      c => c.dataset.timeline === 'live' || c.dataset.timeline === 'upcoming'
    );

    if (targetIdx > 0 && allCards.length > 0) {
      // All cards share the same template so card[0].width ≈ every card's width
      const cardWidth = allCards[0].getBoundingClientRect().width;
      const gap = 16; // matches CSS gap: 16px on .scoreboard-scroll
      // Each card occupies (cardWidth + gap) pixels in the scroll axis.
      // scrollLeft = targetIdx × (cardWidth + gap) puts the card flush at left.
      const targetScrollLeft = targetIdx * (cardWidth + gap);
      scroll.style.scrollBehavior = 'auto';
      scroll.scrollLeft = targetScrollLeft;
      setTimeout(() => { scroll.style.scrollBehavior = 'smooth'; }, 50);
    }
  }, 500);
}
