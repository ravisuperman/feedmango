...
function renderScoreboardTicker(allMatches) {
  ...
  // Use double-rAF to ensure DOM is painted and layout is computed.
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      if (firstFocus) {
        // getBoundingClientRect gives positions relative to viewport, not scroll container.
        // We need the card's left offset relative to the scroll container itself.
        const containerRect = scroll.getBoundingClientRect();
        const cardRect = firstFocus.getBoundingClientRect();
        const offsetFromContainer = cardRect.left - containerRect.left;
        
        // Snap without smooth animation first (instant jump for initial load)
        scroll.style.scrollBehavior = 'auto';
        scroll.scrollLeft += offsetFromContainer - 52; // 52px = arrow button width + padding
        
        // Re-enable smooth for user arrow clicks
