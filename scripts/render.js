...
function renderScoreboardTicker(allMatches) {
  ...
  // We use setTimeout(500ms) to guarantee the browser has fully resolved
  // all flex item widths before we read offsetLeft.
  // NOTE: .scoreboard-scroll must have position:relative (set in widgets.css)
  // so that cards' offsetParent resolves to the scroll container itself.
  setTimeout(() => {
    const allCards = Array.from(scroll.querySelectorAll('.score-card'));
    const firstFocus = allCards.find(c => c.dataset.timeline === 'live') ||
                       allCards.find(c => c.dataset.timeline === 'upcoming');

    if (firstFocus) {
      // offsetLeft is now measured correctly relative to .scoreboard-scroll
      // because position:relative is set on it.
      // Subtract 48px for the left padding of the scroll container.
      const targetScroll = Math.max(0, firstFocus.offsetLeft - 48);
