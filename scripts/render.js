...
function renderScoreboardTicker(allMatches) {
  ...
  // ── CLIENT-SIDE IPL SAFETY FILTER + COMPLETED FILTER ─────────────────────
  // Only show LIVE and UPCOMING matches. Completed matches are excluded entirely
  // so the first visible card is always today's or the next scheduled match.
  const isCompleted = (status) => {
    const s = (status || '').toUpperCase();
    return s.includes('WON') || s.includes('DRAW') || s.includes('ABANDONED') || s.includes('TIED') || s === 'FT';
  };

  const matches = allMatches.filter(m => {
    const title = (m.name || '').toLowerCase();
    const isIPL = title.includes('ipl') || title.includes('indian premier league');
    return isIPL && !isCompleted(m.status);
  });
  ...
  // ── CHRONOLOGICAL SORT: Live first, then Upcoming by date ──────────────────
  ...
  // the first card is always the live or next upcoming match.
  scroll.scrollLeft = 0;
}
