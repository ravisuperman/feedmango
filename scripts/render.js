...
function renderScoreboardTicker(allMatches) {
  ...
  // Use a 300ms timeout — long enough for the browser to fully resolve all
  // flex item widths and offsetLeft values inside the scroll container.
  setTimeout(() => {
    const firstFocus =
    ...
    if (firstFocus) {
      ...
      // offsetLeft is now reliable — it gives the px distance from scroll container start
      scroll.scrollLeft = Math.max(0, firstFocus.offsetLeft - 48);
      // Re-enable smooth for arrow navigation
